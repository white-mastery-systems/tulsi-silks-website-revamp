...
| **19 → 20** | `upgrade/angular-20` | `ng update` to v20 (**@angular/core** **^20.3.19**, **cli** / **build-angular** / **@angular/ssr** **^20.3.25**). **Node.js** minimum **20.19+** or **22.12+** (CLI refuses older Node); **`package.json`** documents **`engines.node`**. CLI migrations: root **`tsconfig.json`** **`moduleResolution`: `"bundler"`**; **`angular.json`** workspace generation defaults. Core: **`DOCUMENT`** imports moved **`@angular/common` → `@angular/core`** (14 files). Third-party: `@angular-slider/ngx-slider` **^20.0.0**, `@angular/youtube-player` **^20.2.14**, `angularx-qrcode` **^20.0.0**, `ngx-bootstrap` **^20.0.2**. Verified (Node **22.17**): `ng build --configuration=development`, `ng run ecommerce:server:development`. |

Next: commit on branch `upgrade/angular-20`, push, PR → merge, then `upgrade/angular-21` (or next LTS) for the following hop. **Use Node 20.19+ or 22.12+** locally and in CI before building. **Smoke-test** SSR and key flows after deploy.
